...
class UserDAO {
    ...
    createUser(username: string, name: string, surname: string, password: string, role: string): Promise<boolean> {
        return new Promise<boolean>((resolve, reject) => {
            try {
                ...
                db.run(sql, [username, name, surname, role, hashedPassword, salt], (err: Error | null, row: any) => {
                    if (err) {
        ...
        return new Promise<User[]>((resolve,reject)=>{
            ...
            db.all(sql,(err: Error | null, rows: any)=>{
                if(err){
                    ...
                    const users = rows.map((user: { username: string; name: string; surname: string; role: Role; address: string; birthdate: string; })=> new User(user.username, user.name,user.surname,user.role,user.address,user.birthdate))
                    resolve(users)
                }
    ...
    deleteByUsername(user:User, username:string):Promise<Boolean>{
        return new Promise<Boolean>((resolve,reject)=>{
    ...
    updateUser(user:User,name:string,surname:string,address:string,birthdate:string,username:string,flag:Boolean):Promise<User>{
        return new Promise<User>((resolve,reject)=>{
            try{
                ...
                const adminSQL = `SELECT * FROM users WHERE username=?`
                db.get(adminSQL,[username],(err:Error|null,row:any)=>{
                    if (err){
                        reject(err)
                    }
                    if (row){
                        if(row.role==="Admin" && user.username!=row.username){
                            ...
                            db.run(sql,[name,surname,address,birthdate,username],function(this:RunResult,err:Error|null){
                               ...
                               if (this.changes){
                                   ...
                                   db.get(retrieveSQL,[username],(err:Error|null,row:any)=>{
                                       if(err){
                                           reject(err)
                                       }
                                       if (!row) {
