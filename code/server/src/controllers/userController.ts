...
class UserController {
    ...
    async createUser(username: string, name: string, surname: string, password: string, role: string):Promise<Boolean> {
        return new Promise<Boolean>(async(resolve,reject)=>{
            try{
                const createUserResult = await this.dao.createUser(username,name,surname,password,role);
                if (createUserResult){
                 resolve(true)}
            }
            catch(error){
    ...
    async getUsers():Promise<User[]>{
        return new Promise<User[]>(async(resolve,reject)=>{
            try{
                const getUsersResult = await this.dao.getAllUsers();
                if (getUsersResult){
    ...
    async getUsersByRole(role: string):Promise<User[]> { 
        return new Promise<User[]>(async(resolve,reject)=>{
            try{
                const getUsersResult = await this.dao.getUsersByRole(role);
                if (getUsersResult){
                    resolve(getUsersResult)
                }
            }
        ...
        return new Promise<User>(async(resolve,reject)=>{
            try{
                if (Utility.isAdmin(user) || user.username===username){
                    ...
                    const notAllowed = await this.dao.getUserByUsername("Not Admin")
                }
            }
            catch(error){
                reject(error)
    ...
    async deleteUser(user: User, username: string):Promise<Boolean> {
        return new Promise<Boolean>(async(resolve,reject)=>{
            try{
                const deleteResult = await this.dao.deleteByUsername(user,username)
                if (deleteResult){
                    resolve(true)
                }
            }
    ...
    async deleteAll():Promise<Boolean> {
        return new Promise<Boolean>(async(resolve,reject)=>{
            try{
                const deleteAllResult = await this.dao.deleteAll()
                if (deleteAllResult){
                    resolve(true)
                }
            }
    ...
    async updateUserInfo(user: User, name: string, surname: string, address: string, birthdate: string, username: string):Promise<User> {
        return new Promise<User>(async(resolve,reject)=>{
            try{
                let flag=true //not allowed to access
                if (Utility.isAdmin(user) || user.username===username){
                    flag = false
                }
                const updateResult = await this.dao.updateUser(user,name,surname,address,birthdate,username,flag)
                if (updateResult){
